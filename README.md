# 企业微信作业管理机器人

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Node.js](https://img.shields.io/badge/Node.js-18+-green.svg)](https://nodejs.org/)

一个基于企业微信的教学作业管理系统，支持作业同步、自动提醒、消息转发等功能。

## ⚠️ 重要说明：关于建群

**企业微信官方 API 不支持自动创建外部客户群。** 老师需要在企业微信 App 中手动创建群聊，然后机器人才能加入并提供服务。

这是企业微信的官方限制，不是本项目的问题。详见下方「使用流程」。

## ✨ 功能特性

- 📚 **作业同步** - 自动识别并保存作业消息到 Redis
- ⏰ **定时提醒** - 每日 18:00/20:00 自动提醒学生完成作业
- 💬 **自动回复** - 智能回复常见问题
- 📱 **Telegram 转发** - 将企业微信消息同步到 Telegram
- 🔗 **群二维码** - 获取客户群入群二维码（需要「客户联系」权限）

## 📋 使用流程

### 第一步：手动创建班级群（必须）

由于企业微信官方 API 限制，**外部客户群必须手动创建**：

1. 打开企业微信 App（手机端）
2. 点击右上角「+」按钮
3. 选择「发起群聊」
4. 选择「选择客户」或「外部群」
5. 添加老师（企业员工）和学生/家长（外部联系人）
6. 点击「确定」完成创建

> 💡 **为什么不能自动建群？**
> 
> 企业微信将群聊分为两种：
> - **内部群**：只能包含企业员工，可以通过 API 创建
> - **客户群**：可以包含外部联系人（学生/家长），**只能手动创建**
> 
> 教学场景需要学生/家长加入，因此必须使用客户群，需要手动创建。

### 第二步：机器人自动服务

群创建后，机器人自动提供以下功能：

| 功能 | 说明 |
|------|------|
| 作业同步 | 老师发送作业消息，自动保存到系统 |
| 自动回复 | 回复「你好」「帮助」「状态」等常见问题 |
| 定时提醒 | 每日 18:00/20:00 自动提醒学生完成作业 |
| 消息转发 | 将群消息同步到 Telegram 便于管理 |

### 第三步：发布作业

老师在群内发送以下格式的消息，系统自动识别为作业：

```
【作业】今天背诵第三课单词，完成练习册P15-16

#作业 预习明天的课文

今日作业：完成阅读理解练习
```

### 第四步：获取群二维码（可选）

如果需要邀请更多学生/家长入群，可以：

**方式一：企业微信 App 操作**
1. 打开群聊
2. 点击右上角「...」
3. 点击「群二维码」
4. 分享给学生/家长

**方式二：通过 API 获取（需要「客户联系」权限）**
```bash
# 获取所有客户群
curl http://你的域名/api/customer-groups

# 生成指定群的入群二维码
curl -X POST http://你的域名/api/customer-groups/{chatId}/qrcode
```

## 🚀 快速开始

### 环境要求

- Node.js >= 18
- Redis >= 6
- 企业微信自建应用

### 安装

```bash
# 克隆仓库
git clone https://github.com/ruiyangruiyi/wecom-homework-bot.git
cd wecom-homework-bot

# 安装依赖
npm install

# 配置环境变量
cp .env.example .env
# 编辑 .env 填入你的配置
```

### 配置

创建 `.env` 文件：

```env
# 企业微信配置
WECOM_CORP_ID=你的企业ID
WECOM_AGENT_ID=应用AgentID
WECOM_SECRET=应用Secret
WECOM_TOKEN=回调Token
WECOM_AES_KEY=回调EncodingAESKey

# Telegram 转发（可选）
TELEGRAM_BOT_TOKEN=你的Bot Token
TELEGRAM_CHAT_ID=目标群组ID

# 服务端口
PORT=3000
```

### 启动

```bash
# 开发模式
node server.js

# 生产模式（推荐使用 PM2）
pm2 start server.js --name wecom-bot
```

## 📖 API 文档

### 回调接口

企业微信回调地址配置为：`http://你的域名/api/wecom/callback`

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/wecom/callback | URL 验证接口 |
| POST | /api/wecom/callback | 消息接收接口 |

### 消息发送

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/wecom/send | 发送企业微信消息 |

**请求体：**
```json
{
  "touser": "用户ID或@all",
  "content": "消息内容"
}
```

### 客户群管理（需要「客户联系」权限）

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/customer-groups | 获取所有客户群列表 |
| POST | /api/customer-groups/:chatId/qrcode | 生成指定群的入群二维码 |

### 内部群管理

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/group/create | 创建企业内部群（仅限企业员工） |

### 健康检查

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /health | 返回服务状态信息 |

## 💬 指令说明

在企业微信中发送以下指令：

| 指令 | 说明 | 示例 |
|------|------|------|
| `你好` | 打招呼 | 你好 |
| `帮助` | 查看帮助 | 帮助 |
| `状态` | 查看系统状态 | 状态 |
| `建群 群名 成员` | 创建内部群（仅限员工） | 建群 教师群 ZhangChong,YangNai |

## 📚 作业识别规则

系统自动识别以下格式的作业消息：

| 格式 | 示例 |
|------|------|
| `【作业】...` | 【作业】背诵课文 |
| `#作业 ...` | #作业 完成练习册 |
| `今日作业：...` | 今日作业：预习第五课 |
| `作业布置：...` | 作业布置：写作文一篇 |
| `布置作业 ...` | 布置作业 复习单词 |
| `homework: ...` | homework: read chapter 3 |

## 🏗️ 项目结构

```
wecom-homework-bot/
├── server.js           # 主服务入口
├── homework-sync.js    # 作业同步模块
├── homework-reminder.js # 作业提醒模块
├── package.json        # 项目配置
├── .env.example        # 环境变量示例
└── README.md           # 项目文档
```

## 🔧 部署指南

### 使用 PM2 部署

```bash
# 安装 PM2
npm install -g pm2

# 启动服务
pm2 start server.js --name wecom-bot

# 设置开机自启
pm2 startup
pm2 save
```

### 使用 Nginx 反向代理

```nginx
server {
    listen 80;
    server_name your-domain.com;

    location /api/wecom/callback {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }

    location / {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_cache_bypass $http_upgrade;
    }
}
```

## ⚠️ 已知限制

| 限制 | 说明 | 解决方案 |
|------|------|----------|
| 外部群创建 | 企业微信 API 不支持创建客户群 | 手动在 App 中创建 |
| 群二维码 | 需要「客户联系」权限 | 在企业微信后台开启权限 |
| 消息接收 | 群内普通消息需要配置权限 | 配置「接收消息」权限 |

## 📝 更新日志

查看 [CHANGELOG.md](./CHANGELOG.md)

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📄 许可证

[MIT License](./LICENSE)
